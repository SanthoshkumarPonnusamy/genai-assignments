...
export const DEFAULT_PROMPTS = {
  ...
  SELENIUM_CSHARP_PAGE_ONLY: `
    ...
    - Add XML comments for methods & class.
    - Use Selenium WebDriver 4+ compatible imports.
    - Use meaningful method names.
    - Do NOT include explanations or test code.
    ...
    Output Format:
    - A single C# class inside a \`\`\`csharp\`\`\` block.
  ...
  /**
   * Cucumber with Step Definitions for Selenium C#
   */
  CUCUMBER_WITH_SELENIUM_CSHARP_STEPS: `
    Instructions:
    - Generate BOTH:
      1. A Cucumber .feature file.
      2. A C# step definition class for Selenium.
    - Do NOT include Page Object code.
    - Step defs must include WebDriver setup, explicit waits, and actual Selenium code.
    - Use Scenario Outline with Examples table (South India realistic data).
    ...
    namespace TestLeaf.StepDefinitions
    {
        [Binding]
        public class LoginStepDefinitions
        {
            ...
            public void EnterUsername(string username)
            {
                var el = wait.Until(ExpectedConditions.ElementToBeClickable(By.Id("username")));
                el.SendKeys(username);
            }
            ...
            public void EnterPassword(string password)
            {
                var el = wait.Until(ExpectedConditions.ElementToBeClickable(By.Id("password")));
                el.SendKeys(password);
            }
            ...
            public void VerifyLogin()
            {
                var success = wait.Until(ExpectedConditions.ElementIsVisible(By.ClassName("success")));
    ...
    Persona:
    - Audience: QA engineers working with SpecFlow & Selenium.

    Output Format:
    - Gherkin in \`\`\`gherkin\`\`\` block + C# code in \`\`\`csharp\`\`\` block.
